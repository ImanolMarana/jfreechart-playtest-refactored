...
public class Week extends RegularTimePeriod implements Serializable {
    ...
    public static Week parseWeek(String s) {
        ...
        if (s != null) {
            ...
            int separatorIndex = findSeparator(s);
            if (separatorIndex != -1) {
                result = parseWeekWithSeparator(s, separatorIndex);
            } else {
    ...
    private static Week parseWeekWithSeparator(String s, int separatorIndex) {
        String s1 = s.substring(0, separatorIndex).trim();
        String s2 = s.substring(separatorIndex + 1, s.length()).trim();
        ...
        if (year != null) {
            ...
            return parseWeekWithYearInSecondPart(s1, s2);
        }
    }

    private static Week parseWeekWithYearInSecondPart(String s1, String s2) {
        Year year = evaluateAsYear(s2);
        if (year != null) {
            int week = stringToWeek(s1);
            if (week == -1) {
                throw new TimePeriodFormatException("Can't evaluate the week.");
            }
            return new Week(week, year);
