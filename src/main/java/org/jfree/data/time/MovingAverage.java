...
public class MovingAverage {
        ...
        if (source.getItemCount() > 0) {
            ...
            for (int i = source.getItemCount() - 1; i >= 0; i--) {
                ...
                if (serial >= firstSerial) {
                    result.add(period, calculateAverage(source, i, periodCount, serial));
                }
            }
        }
        return result;
    ...
    private static <S extends Comparable<S>> Double calculateAverage(TimeSeries<S> source, int index, int periodCount, long serial) {
        int n = 0;
        double sum = 0.0;
        long serialLimit = source.getTimePeriod(index).getSerialIndex() - periodCount;
        int offset = 0;
        ...
        while (offset < periodCount) {
            ...
            if (sourceIndex >= 0) {
                TimeSeriesDataItem item = source.getRawDataItem(sourceIndex);
                long currentIndex = item.getPeriod().getSerialIndex();
                ...
                if (currentIndex > serialLimit) {
                    ...
                    break; // No need to continue if we are below the serial limit
                }
            } else {
