...
public class TimeSeries<S extends Comparable<S>> extends Series<S> 
    ...
    public void add(TimeSeriesDataItem item, boolean notify) {
        ...
        if (this.timePeriodClass == null) {
            ...
            throw new SeriesException(
                "You are trying to add data where the time period class "
                + "is " + item.getPeriod().getClass().getName()
        ...
        if (count == 0) {
            ...
            added = handleNonEmptySeriesAdd(item);
        }
        if (added) {
    ...
    private boolean handleNonEmptySeriesAdd(TimeSeriesDataItem item) {
        RegularTimePeriod last = getTimePeriod(getItemCount() - 1);
