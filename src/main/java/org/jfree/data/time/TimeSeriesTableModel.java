...
public class TimeSeriesTableModel extends AbstractTableModel
    ...
    if (row < this.series.getItemCount()) {
      return getValueAtExistingRow(column, row); 
    }
    else {
      return getValueAtNewRow(column);
    }
  }
  
  private Object getValueAtExistingRow(int column, int row) {
    if (column == 0) {
  ...
  private Object getValueAtNewRow(int column) {
    if (column == 0) {
