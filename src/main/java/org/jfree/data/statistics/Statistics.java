...
public abstract class Statistics {
    ...
    public static double calculateMedian(List values, boolean copyAndSort) {
        ...
        if (values != null) {
          if (copyAndSort) {
            values = createSortedCopy(values);
          }
          int count = values.size();
          if (count > 0) {
            result = calculateMedianFromSortedList(values, count);
          }
        }
        return result;
      }

      private static List createSortedCopy(List values) {
      ...
      private static double calculateMedianFromSortedList(List values, int count) {
        double result;
        if (count % 2 == 1) {
          if (count > 1) {
