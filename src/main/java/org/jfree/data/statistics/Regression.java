...
public abstract class Regression {
    ...
    public static double[] getPolynomialRegression(XYDataset dataset, 
        ...
        matrix = calculateSubMatrix(matrix);
        for (int eq = equations - 1; eq > -1; eq--) {
            result[eq] = calculateCoefficient(matrix, result, eq);
        }
        result[equations] = calculateRSquare(data, validItems, result,
                equations, sumY);
        return result;
    }

    private static double calculateRSquare(double[][] data, int validItems,
           double[] result, int equations, double sumY) {
        ...
        return yRegSquare / yObsSquare;
    }

    private static double calculateCoefficient(double[][] matrix, double[] result,
           int eq) {
