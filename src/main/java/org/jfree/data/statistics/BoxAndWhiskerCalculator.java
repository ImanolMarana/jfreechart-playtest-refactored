...
public abstract class BoxAndWhiskerCalculator {
    ...
    List<Number> vlist = stripNullAndNaNItems ? extractValidNumbers(values) : new ArrayList<>(values);
    ...
    double upperOutlierThreshold = calculateOutlierThreshold(q3, interQuartileRange, 1.5);
    double lowerOutlierThreshold = calculateOutlierThreshold(q1, interQuartileRange, -1.5);

    double upperFaroutThreshold = calculateOutlierThreshold(q3, interQuartileRange, 2.0);
    double lowerFaroutThreshold = calculateOutlierThreshold(q1, interQuartileRange, -2.0);

    return calculateBoxAndWhiskerItem(vlist, mean, median, q1, q3, 
            lowerOutlierThreshold, upperOutlierThreshold, lowerFaroutThreshold, upperFaroutThreshold);
}

private static List<Number> extractValidNumbers(List<? extends Number> values) {
    List<Number> vlist = new ArrayList<>(values.size());
...
private static double calculateOutlierThreshold(double quartile, double interQuartileRange, double multiplier) {
    return quartile + (interQuartileRange * multiplier);
}

private static BoxAndWhiskerItem calculateBoxAndWhiskerItem(List<Number> vlist, double mean, double median, double q1,
        double q3, double lowerOutlierThreshold, double upperOutlierThreshold, double lowerFaroutThreshold,
