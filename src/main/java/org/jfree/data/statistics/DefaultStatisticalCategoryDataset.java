...
public class DefaultStatisticalCategoryDataset<R extends Comparable<R>, 
    ...
    private void updateBounds() {
        ...
        for (int r = 0; r < rowCount; r++) {
            for (int c = 0; c < columnCount; c++) {
                updateBoundsForCell(r, c);
            }
        }
    }
    
    private void updateBoundsForCell(int row, int column) {
        ...
        updateMaxValues(m, sd, row, column);
        updateMinValues(m, sd, row, column);
    }

    private void updateMaxValues(double mean, double sd, int row, int column) {
        if (!Double.isNaN(mean)) {
            if (Double.isNaN(this.maximumRangeValue) || mean > this.maximumRangeValue) {
        ...
        if (!Double.isNaN(sd) && !Double.isNaN(mean)) {
            double value = mean + sd;
            if (Double.isNaN(this.maximumRangeValueIncStdDev) || value > this.maximumRangeValueIncStdDev) {
                this.maximumRangeValueIncStdDev = value;
    ...
    private void updateMinValues(double mean, double sd, int row, int column) {
        if (!Double.isNaN(mean)) {
            if (Double.isNaN(this.minimumRangeValue) || mean < this.minimumRangeValue) {
        ...
        if (!Double.isNaN(sd) && !Double.isNaN(mean)) {
            double value = mean - sd;
            if (Double.isNaN(this.minimumRangeValueIncStdDev) || value < this.minimumRangeValueIncStdDev) {
                this.minimumRangeValueIncStdDev = value;
