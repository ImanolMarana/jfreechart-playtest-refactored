...
public class DefaultWindDataset extends AbstractXYDataset
    ...
    public DefaultWindDataset(List seriesKeys, Object[][][] data) {
        ...
        this.allSeriesData = new java.util.ArrayList(data.length);

        for (int seriesIndex = 0; seriesIndex < data.length; seriesIndex++) {
            this.allSeriesData.add(seriesIndex, createSeriesData(data[seriesIndex]));
        }

    }
    
    private List createSeriesData(Object[][] seriesData) {
        List oneSeriesData = new java.util.ArrayList(seriesData.length);
        for (int itemIndex = 0; itemIndex < seriesData.length; itemIndex++) {
            Object xObject = seriesData[itemIndex][0];
            if (xObject != null) {
                Number xNumber;
                ...
                Number windDir = (Number) seriesData[itemIndex][1];
                Number windForce = (Number) seriesData[itemIndex][2];
