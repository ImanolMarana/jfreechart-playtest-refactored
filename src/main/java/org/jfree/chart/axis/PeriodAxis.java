...
public class PeriodAxis extends ValueAxis
    ...
    protected AxisState drawTickLabels(int band, Graphics2D g2, AxisState state,
        ...
        double delta1 = calculateInitialGap(g2, edge, band);
        ...
        // calculate label positioning parameters
        long axisMin = this.first.getFirstMillisecond();
        long axisMax = this.last.getLastMillisecond();
        RegularTimePeriod labelPeriod = calculateLabelPeriod(axisMin, axisMax, band);
        int periodsToSkip = calculatePeriodsToSkip(g2, dataArea, edge, axisMin,
                axisMax, labelPeriod);

        // draw the labels
        drawLabels(g2, state, dataArea, edge, band, axisMin, axisMax,
                labelPeriod, periodsToSkip);

        return state;
    }

    private double calculateInitialGap(Graphics2D g2, RectangleEdge edge, int band) {
        FontMetrics fm = g2.getFontMetrics(this.labelInfo[band].getLabelFont());
        if (edge == RectangleEdge.BOTTOM) {
            return this.labelInfo[band].getPadding().calculateTopOutset(
                    fm.getHeight());
        } else if (edge == RectangleEdge.TOP) {
            return this.labelInfo[band].getPadding().calculateBottomOutset(
                    fm.getHeight());
        }
        return 0.0;
    }

    private RegularTimePeriod calculateLabelPeriod(long axisMin, long axisMax, int band) {
        return this.labelInfo[band].createInstance(
                new Date((axisMin + axisMax) / 2), this.timeZone, this.locale);
    }

    private int calculatePeriodsToSkip(Graphics2D g2, Rectangle2D dataArea,
                                      RectangleEdge edge, long axisMin, long axisMax,
                                      RegularTimePeriod period) {
        DateFormat df = this.labelInfo[band].getDateFormat();
        df.setTimeZone(this.timeZone);
        String label = df.format(new Date(period.getMiddleMillisecond()));
        Rectangle2D b = TextUtils.getTextBounds(label, g2, g2.getFontMetrics());
        double w = b.getWidth();
        ...
        long length = period.getLastMillisecond() - period.getFirstMillisecond();
        return (int) (ww / length) + 1;
    }

    private void drawLabels(Graphics2D g2, AxisState state, Rectangle2D dataArea,
                            RectangleEdge edge, int band, long axisMin, long axisMax,
                            RegularTimePeriod p, int periodsToSkip) {
        Rectangle2D b = null;
        long lastXX = 0L;
        float y = (float) (state.getCursor());
        TextAnchor anchor = getLabelAnchor(edge);
        float yDelta = calculateLabelYDelta(g2, edge);

        while (p.getFirstMillisecond() <= axisMax) {
            float x = (float) valueToJava2D(p.getMiddleMillisecond(), dataArea,
                    edge);
            String label = getLabelText(g2, dataArea, edge, p, axisMin, axisMax);
            ...
            if (lastXX > 0L && this.labelInfo[band].getDrawDividers()) {
                drawDivider(g2, dataArea, edge, band, y, yDelta, lastXX, p);
            }

            lastXX = p.getLastMillisecond();
            for (int i = 0; i < periodsToSkip; i++) {
                p = p.next();
            }
            p.peg(this.calendar);
        }

        updateStateCursor(state, edge, b);
    }

    private TextAnchor getLabelAnchor(RectangleEdge edge) {
        return edge == RectangleEdge.TOP ? TextAnchor.BOTTOM_CENTER
                : TextAnchor.TOP_CENTER;
    }

    private float calculateLabelYDelta(Graphics2D g2, RectangleEdge edge) {
        return edge == RectangleEdge.TOP
                ? -(float) g2.getFontMetrics().getHeight()
                : (float) g2.getFontMetrics().getHeight();
    }

    private String getLabelText(Graphics2D g2, Rectangle2D dataArea,
                                 RectangleEdge edge, RegularTimePeriod p,
                                 long axisMin, long axisMax) {
        DateFormat df = this.labelInfo[band].getDateFormat();
        df.setTimeZone(this.timeZone);
        ...
        if (last > axisMax) {
            label = handlePartiallyVisibleLastLabel(g2, dataArea, edge, label,
                    first, axisMin);
        } else if (first < axisMin) {
            label = handlePartiallyVisibleFirstLabel(g2, dataArea, edge, label,
                    last, axisMax);
        }
        return label;
    }

    private String handlePartiallyVisibleLastLabel(Graphics2D g2,
                                                    Rectangle2D dataArea, RectangleEdge edge, String label,
                                                    long first, long axisMin) {
    ...
    private String handlePartiallyVisibleFirstLabel(Graphics2D g2,
                                                    Rectangle2D dataArea, RectangleEdge edge, String label,
                                                    long last, long axisMax) {
        Rectangle2D bb = TextUtils.getTextBounds(label, g2,
                g2.getFontMetrics());
    ...
    private void drawDivider(Graphics2D g2, Rectangle2D dataArea,
                             RectangleEdge edge, int band, float y, float yDelta,
                             long lastXX, RegularTimePeriod p) {
    ...
    private void updateStateCursor(AxisState state, RectangleEdge edge,
                                  Rectangle2D b) {
        double used = 0.0;
        if (b != null) {
            used = b.getHeight();
            ...
            if (edge == RectangleEdge.BOTTOM) {
                used += this.labelInfo[band].getPadding()
                        .calculateBottomOutset(fm.getHeight());
            } else if (edge == RectangleEdge.TOP) {
                used += this.labelInfo[band].getPadding()
                        .calculateTopOutset(fm.getHeight());
            }
        }
