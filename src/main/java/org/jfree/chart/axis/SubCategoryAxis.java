...
public class SubCategoryAxis extends CategoryAxis
    ...
    protected AxisState drawSubCategoryLabels(Graphics2D g2,
        ...
        for (int categoryIndex = 0; categoryIndex < categoryCount;
            ...
            Rectangle2D area = calculateSubCategoryArea(categoryIndex, categoryCount, dataArea, edge, state, maxdim);

            int subCategoryCount = this.subCategories.size();
            float width = (float) ((area.getWidth()) / subCategoryCount);
            float height = (float) ((area.getHeight()) / subCategoryCount);
            float xx, yy;
            for (int i = 0; i < subCategoryCount; i++) {
                if (RectangleEdge.isTopOrBottom(edge)) {
                    xx = (float) (area.getX() + (i + 0.5) * width);
                    yy = (float) area.getCenterY();
                }
                else {
                    xx = (float) area.getCenterX();
                    yy = (float) (area.getY() + (i + 0.5) * height);
                }
    ...
    private Rectangle2D calculateSubCategoryArea(int categoryIndex, int categoryCount, Rectangle2D dataArea, RectangleEdge edge, AxisState state, double maxdim) {
        double x0 = 0.0;
        double x1 = 0.0;
        double y0 = 0.0;
        double y1 = 0.0;
        ...
        return new Rectangle2D.Double(x0, y0, (x1 - x0),
                (y1 - y0));
    }
//Refactoring end
    }
