...
public class SymbolAxis extends NumberAxis implements Serializable {
    ...
    protected void autoAdjustRange() {
        ...
        if (plot instanceof ValueAxisPlot) {
            ...
            range = Math.max(range, getAutoRangeMinimumSize());
           
            // Calculate margins based on includeZero and stickyZero 
            double upperMargin = calculateUpperMargin(range);
            double lowerMargin = calculateLowerMargin(range);

            // Apply margins to upper and lower bounds
            upper += upperMargin;
            lower -= lowerMargin;
    ...
    private double calculateUpperMargin(double range) {
    ...
    private double calculateLowerMargin(double range) {
        if (getAutoRangeIncludesZero()) {
            if (getAutoRangeStickyZero()) {
                return 0.0;
            } else {
