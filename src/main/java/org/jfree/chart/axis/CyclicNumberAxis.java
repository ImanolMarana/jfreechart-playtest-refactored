...
public class CyclicNumberAxis extends NumberAxis {
    ...
    public double valueToJava2D(double value, Rectangle2D dataArea,
        ...
        double jmin = getJMin(dataArea, edge);
        double jmax = getJMax(dataArea, edge);

        if (isInverted()) {
            return calculateInvertedJava2D(value, vp, jmin, jmax);
        } else {
            return calculateJava2D(value, vp, jmin, jmax);
        }
      }

      private double getJMin(Rectangle2D dataArea, RectangleEdge edge) {
        if (RectangleEdge.isTopOrBottom(edge)) {
      ...
      private double getJMax(Rectangle2D dataArea, RectangleEdge edge) {
        if (RectangleEdge.isTopOrBottom(edge)) {
      ...
      private double calculateInvertedJava2D(double value, double vp, double jmin,
              double jmax) {
        if (value == vp) {
      ...
      private double calculateJava2D(double value, double vp, double jmin,
              double jmax) {
        if (value == vp) {
