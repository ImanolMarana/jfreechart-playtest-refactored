...
public class NumberAxis extends ValueAxis implements Cloneable, Serializable {
    ...
    protected void autoAdjustRange() {
        ...
        if (plot instanceof ValueAxisPlot) {
            ...
            // Apply fixed auto range
            double fixedAutoRange = getFixedAutoRange();
            if (fixedAutoRange > 0.0) {
                lower = upper - fixedAutoRange;
            } else {
                // Ensure minimum auto range size
                range = ensureMinimumRangeSize(range, lower, upper);
            }

            // Apply sticky zero and margins
            if (getAutoRangeStickyZero()) {
                upper = applyStickyZeroAndUpperMargin(upper, range);
                lower = applyStickyZeroAndLowerMargin(lower, range);
            } else {
    ...
    private double ensureMinimumRangeSize(double range, double lower, double upper){
    ...
    private double applyStickyZeroAndUpperMargin(double upper, double range){
        if (upper <= 0.0) {
            return Math.min(0.0, upper + getUpperMargin() * range);
        } else {
            return upper + getUpperMargin() * range;
        }
    }
    
    private double applyStickyZeroAndLowerMargin(double lower, double range){
        if (lower >= 0.0) {
            return Math.max(0.0, lower - getLowerMargin() * range);
        } else {
            return lower - getLowerMargin() * range;
        }
    }
//Refactoring end
