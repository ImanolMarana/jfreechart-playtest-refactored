...
public class SpiderWebPlot extends Plot implements Cloneable, Serializable {
        ...
        for (int cat = 0; cat < catCount; cat++) {
            ...
            if (dataValue != null) {
                ...
                if (value >= 0) { // draw the polygon series...
                    drawPolygonSeries(g2, plotArea, centre, entities, series,
                            cat, headH, headW, value);
                }
            }
        }
        // Plot the polygon
    ...
    private void drawPolygonSeries(Graphics2D g2, Rectangle2D plotArea,
                                   Point2D centre, EntityCollection entities, int series, int cat,
                                   double headH, double headW, double value) {
        ...
        if (entities != null) {
            addEntity(entities, series, cat, point, headH, headW);
        }
    }

    private void addEntity(EntityCollection entities, int series, int cat,
                           Point2D point, double headH, double headW) {
        int row, col;
        if (this.dataExtractOrder == TableOrder.BY_ROW) {
            row = series;
            col = cat;
