...
public class ThermometerPlot extends Plot implements ValueAxisPlot,
    ...
    *
    ...
    public void draw(Graphics2D g2, Rectangle2D area, Point2D anchor,
        ...
        // create shapes for thermometer components
        ...
        if ((this.dataset != null) && (this.dataset.getValue() != null)) {
            drawMercury(g2, dataArea, midX, interior, innerBulb, mercuryStem, stemBottom);

            // draw value
            drawValue(g2, midX, stemBottom, current);
        }

        // draw range indicators
        if (this.subrangeIndicatorsVisible) {
            drawSubrangeIndicators(g2, dataArea, midX);
        }

        // draw the axis...
        drawAxis(g2, area, midX, stemTop, stemBottom);

        g2.setPaint(this.thermometerPaint);
        g2.setFont(this.valueFont);

        //  draw units indicator
        drawUnitsIndicator(g2, area, midX);
    ...
    private void drawMercury(Graphics2D g2, Rectangle2D dataArea, int midX, Rectangle2D interior, Ellipse2D innerBulb, RoundRectangle2D mercuryStem, int stemBottom) {
        ...
        if (k < (stemBottom + getBulbRadius())) {
            ...
            Area tempArea = new Area(mercuryStem);
    ...
    private void drawSubrangeIndicators(Graphics2D g2, Rectangle2D dataArea, int midX) {
    ...
    private void drawAxis(Graphics2D g2, Rectangle2D area, int midX, int stemTop, int stemBottom) {
    ...
    private void drawUnitsIndicator(Graphics2D g2, Rectangle2D area, int midX) {
        FontMetrics metrics = g2.getFontMetrics();
