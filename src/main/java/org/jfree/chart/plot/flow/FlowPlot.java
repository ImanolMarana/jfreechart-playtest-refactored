...
public class FlowPlot extends Plot implements Cloneable, PublicCloneable, 
    ...
    protected Color lookupNodeColor(NodeKey nodeKey) {
        ...
        if (result == null) {
            ...
            if (!this.nodeColorSwatch.isEmpty()) {
                result = getNodeColorFromSwatch(nodeKey);
            } else {
    ...
    private Color getNodeColorFromSwatch(NodeKey nodeKey) {
        ...
        Color result = this.nodeColorSwatch.get(Math.min(this.nodeColorSwatchPointer, this.nodeColorSwatch.size() - 1));
