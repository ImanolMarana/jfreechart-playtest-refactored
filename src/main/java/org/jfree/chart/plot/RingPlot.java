...
public class RingPlot extends PiePlot implements Cloneable, Serializable {
    ...
    protected void drawItem(Graphics2D g2, int section, Rectangle2D dataArea,
        ...
        if (Math.abs(angle) > getMinimumArcAngleToDraw()) {
            ...
            GeneralPath path = createSectionPath(arcBounds, angle1, angle);

            Line2D separator = new Line2D.Double(arc.getEndPoint(),
                    arc.getStartPoint());

            switch (currentPass) {
                case 0:
                    drawItemShadow(g2, path);
                    break;
                case 1:
                    drawItemFillAndOutline(g2, section, path, key);
                    drawCenterText(g2, dataArea, n);
                    addSectionEntity(state, path, dataset, key);
                    break;
                case 2:
                    drawSeparators(g2, separator);
                    break;
            }
        }
    ...
    private GeneralPath createSectionPath(Rectangle2D arcBounds, double angle1, double angle) {
        ...
        path.moveTo((float) arc2.getStartPoint().getX(),
                (float) arc2.getStartPoint().getY());
        path.append(arc2.getPathIterator(null), false);
        path.append(new Arc2D.Double(arcBounds, angle1, angle,
                Arc2D.OPEN).getPathIterator(null), true);
        path.closePath();
        return path;
    }

    private void drawItemShadow(Graphics2D g2, Shape path) {
    ...
    private void drawItemFillAndOutline(Graphics2D g2, int section, GeneralPath path, Comparable key) {
    ...
    private void drawCenterText(Graphics2D g2, Rectangle2D dataArea, Number n) {
        if (this.centerTextMode == CenterTextMode.VALUE) {
            String nstr = this.centerTextFormatter.format(n);
            drawCenteredText(g2, dataArea, nstr);
        } else if (this.centerTextMode == CenterTextMode.FIXED) {
            drawCenteredText(g2, dataArea, this.centerText);
        }
    }
    
    private void drawCenteredText(Graphics2D g2, Rectangle2D dataArea, String text) {
        if (text != null) {
            g2.setFont(this.centerTextFont);
            g2.setPaint(this.centerTextColor);
            TextUtils.drawAlignedString(text, g2,
    ...
    private void addSectionEntity(PiePlotState state, Shape path, PieDataset dataset, Comparable key) {
        if (state.getInfo() != null) {
            EntityCollection entities = state.getEntityCollection();
            if (entities != null) {
                String tip = getToolTipGenerator() != null ? getToolTipGenerator().generateToolTip(dataset, key) : null;
                String url = getURLGenerator() != null ? getURLGenerator().generateURL(dataset, key, getPieIndex()) : null;
                PieSectionEntity entity = new PieSectionEntity(path,
                        dataset, getPieIndex(), 0, key, tip,
                        url);
                entities.add(entity);
            }
        }
    ...
    private double getExplodePercent(Comparable key) {
        double ep = 0.0;
    ...
    private void drawSeparators(Graphics2D g2, Line2D separator) {
