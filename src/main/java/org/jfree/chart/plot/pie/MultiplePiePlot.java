...
public class MultiplePiePlot extends Plot implements Cloneable, Serializable {
    ...
    public void draw(Graphics2D g2, Rectangle2D area, Point2D anchor,
        ...
        int[] displayDimensions = calculateDisplayDimensions(pieCount, area);
        int displayCols = displayDimensions[0];
        int displayRows = displayDimensions[1];
    ...
    private int[] calculateDisplayDimensions(int pieCount, Rectangle2D area) {
    ...
    private String getPieTitle(int pieIndex) {
        if (this.dataExtractOrder == TableOrder.BY_ROW) {
            return this.dataset.getRowKey(pieIndex).toString();
        }
        else {
            return this.dataset.getColumnKey(pieIndex).toString();
        }
    }
    
    private PieDataset createPieDataset(int pieIndex) {
    ...
    private void updateSectionPaints(PieDataset piedataset, PiePlot piePlot) {
