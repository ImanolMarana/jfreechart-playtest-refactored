...
public class LCBLayout implements LayoutManager, Serializable {
    ...
    */
    ...
    synchronized (parent.getTreeLock()) {
      ...
      calculateComponentDimensions(parent, nrows);

      int totalHeight = calculateTotalHeight(nrows);
      int totalWidth = this.colWidth[0] + this.colWidth[1] + this.colWidth[2];

      adjustColumnWidth(parent, insets, totalWidth);

      layoutComponents(parent, insets, nrows);
    }

  }

  private void calculateComponentDimensions(Container parent, int nrows) {
    for (int c = 0; c < COLUMNS; c++) {
      for (int r = 0; r < nrows; r++) {
        Component component = parent.getComponent(r * COLUMNS + c);
        Dimension d = component.getPreferredSize();
  ...
  private int calculateTotalHeight(int nrows) {
    int totalHeight = this.vGap * (nrows - 1);
    for (int r = 0; r < nrows; r++) {
      totalHeight = totalHeight + this.rowHeight[r];
    }
  ...
  private void adjustColumnWidth(Container parent, Insets insets, int totalWidth) {
  ...
  private void layoutComponents(Container parent, Insets insets, int nrows) {
    int x = insets.left;
    for (int c = 0; c < COLUMNS; c++) {
      int y = insets.top;
      for (int r = 0; r < nrows; r++) {
        ...
        if (i < parent.getComponentCount()) {
          layoutComponent(parent, x, y, r, c);
        }
  ...
  private void layoutComponent(Container parent, int x, int y, int row, int col) {
    int i = row * COLUMNS + col;
    Component component = parent.getComponent(i);
    Dimension d = component.getPreferredSize();
    int h = d.height;
    int adjust = (this.rowHeight[row] - h) / 2;
    parent.getComponent(i).setBounds(x, y + adjust, this.colWidth[col], h);
  }
