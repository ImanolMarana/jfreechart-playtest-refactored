...
public class CrosshairOverlay extends AbstractOverlay implements Overlay,
    ...
    private Point2D calculateLabelPoint(Line2D line, RectangleAnchor anchor,
          ...
          if (line.getX1() == line.getX2()) {  // vertical
              x = getLabelX(line, anchor, deltaX);
              y = (line.getY1() + line.getY2()) / 2.0;
              y = getLabelYForVerticalLine(anchor, deltaY, y, line); 
          }
          else {  // horizontal
              x = (line.getX1() + line.getX2()) / 2.0;
              x = getLabelXForHorizontalLine(line, anchor, deltaX, x);
              y = line.getY1();
              y = getLabelY(anchor, deltaY, y);
          }
          return new Point2D.Double(x, y);
      }
      
      private double getLabelX(Line2D line, RectangleAnchor anchor, double deltaX){
          double x = line.getX1();
          if (isLeft(anchor)) {
              x = x - deltaX;
          }
      ...
      private double getLabelYForVerticalLine(RectangleAnchor anchor, double deltaY, double y, Line2D line){
          if (isTop(anchor)) {
      ...
      private double getLabelXForHorizontalLine(Line2D line, RectangleAnchor anchor, double deltaX, double x){
          if (isLeft(anchor)) {
      ...
      private double getLabelY(RectangleAnchor anchor, double deltaY, double y){
          if (isTop(anchor)) {
              y = y - deltaY;
          }
          if (isBottom(anchor)) {
      ...
      private boolean isLeft(RectangleAnchor anchor){
          return (anchor == RectangleAnchor.BOTTOM_LEFT
                  || anchor == RectangleAnchor.LEFT
                  || anchor == RectangleAnchor.TOP_LEFT);
      }

      private boolean isRight(RectangleAnchor anchor){
          return (anchor == RectangleAnchor.BOTTOM_RIGHT
                  || anchor == RectangleAnchor.RIGHT
                  || anchor == RectangleAnchor.TOP_RIGHT);
      }

      private boolean isTop(RectangleAnchor anchor){
          return (anchor == RectangleAnchor.TOP_LEFT
                  || anchor == RectangleAnchor.TOP
                  || anchor == RectangleAnchor.TOP_RIGHT);
      }

      private boolean isBottom(RectangleAnchor anchor){
          return (anchor == RectangleAnchor.BOTTOM_LEFT
