...
public class PaintScaleLegend extends Title implements AxisChangeListener,
    ...
    public Object draw(Graphics2D g2, Rectangle2D area, Object params) {
        ...
        drawBackground(g2, target);
        getFrame().draw(g2, target);
        getFrame().getInsets().trim(target);
        target = trimPadding(target);
        
        if (RectangleEdge.isTopOrBottom(getPosition())) {
            drawHorizontalStrip(g2, target);
        } else {
            drawVerticalStrip(g2, target);
        }
        return null;
    }

    private void drawBackground(Graphics2D g2, Rectangle2D target) {
    ...
    private void drawHorizontalStrip(Graphics2D g2, Rectangle2D target) {
        ...
        if (axisEdge == RectangleEdge.TOP) {
            drawHorizontalStripTop(g2, target, base, increment, r);
        } else if (axisEdge == RectangleEdge.BOTTOM) {
            drawHorizontalStripBottom(g2, target, base, increment, r);
        }
    }

    private void drawHorizontalStripTop(Graphics2D g2, Rectangle2D target, double base, 
                                        double increment, Rectangle2D r) {
        ...
        drawStripOutline(g2, target,  target.getMinX(), target.getMaxY() - this.stripWidth);
        this.axis.draw(g2, target.getMaxY() - this.stripWidth
                - this.axisOffset, target, target, RectangleEdge.TOP,
                null);
    }

    private void drawHorizontalStripBottom(Graphics2D g2, Rectangle2D target, 
                                        double base, double increment, Rectangle2D r) {
        for (int i = 0; i < this.subdivisions; i++) {
            double v = base + (i * increment);
            Paint p = this.scale.getPaint(v);
            double vv0 = this.axis.valueToJava2D(v, target,
        ...
        drawStripOutline(g2, target, target.getMinX(), target.getMinY());
        this.axis.draw(g2, target.getMinY() + this.stripWidth
                + this.axisOffset, target, target,
                RectangleEdge.BOTTOM, null);
    }
    
    private void drawVerticalStrip(Graphics2D g2, Rectangle2D target) {
        double base = this.axis.getLowerBound();
        double increment = this.axis.getRange().getLength() / this.subdivisions;
        Rectangle2D r = new Rectangle2D.Double();
        RectangleEdge axisEdge = Plot.resolveRangeAxisLocation(
        ...
        if (axisEdge == RectangleEdge.LEFT) {
            drawVerticalStripLeft(g2, target, base, increment, r);
        } else if (axisEdge == RectangleEdge.RIGHT) {
            drawVerticalStripRight(g2, target, base, increment, r);
        }
    }
    
    private void drawVerticalStripLeft(Graphics2D g2, Rectangle2D target, double base, 
                                        double increment, Rectangle2D r) {
        for (int i = 0; i < this.subdivisions; i++) {
            double v = base + (i * increment);
            Paint p = this.scale.getPaint(v);
        ...
        drawStripOutline(g2, target, target.getMaxX() - this.stripWidth, target.getMinY());
        this.axis.draw(g2, target.getMaxX() - this.stripWidth
                - this.axisOffset, target, target, RectangleEdge.LEFT,
                null);
    }
    
    private void drawVerticalStripRight(Graphics2D g2, Rectangle2D target, 
                                        double base, double increment, Rectangle2D r) {
        for (int i = 0; i < this.subdivisions; i++) {
            double v = base + (i * increment);
            Paint p = this.scale.getPaint(v);
    ...
    private void drawStripOutline(Graphics2D g2, Rectangle2D target, double x, double y) {
