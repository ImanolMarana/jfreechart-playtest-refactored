...
public class DirectionalGradientPaintTransformer implements GradientPaintTransformer {
    ...
    public GradientPaint transform(GradientPaint paint, Shape target) {
        ...
        // Calculate transformed gradient points
        float[] transformedPoints = calculateTransformedPoints(px1, py1, px2, py2, bx, by, bw, bh,
                paint.isCyclic());

        return new GradientPaint(transformedPoints[0], transformedPoints[1], paint.getColor1(),
                transformedPoints[2], transformedPoints[3], paint.getColor2(),
                paint.isCyclic());
    }

    /**
     * Calculates the transformed gradient points based on original points and target bounds.
     *
     * @param px1     Original gradient point 1 x-coordinate.
     * @param py1     Original gradient point 1 y-coordinate.
     * @param px2     Original gradient point 2 x-coordinate.
     * @param py2     Original gradient point 2 y-coordinate.
     * @param bx      Target bounds x-coordinate.
     * @param by      Target bounds y-coordinate.
     * @param bw      Target bounds width.
     * @param bh      Target bounds height.
     * @param cyclic Whether the gradient is cyclic.
     * @return Array containing transformed points [rx1, ry1, rx2, ry2].
     */
    private float[] calculateTransformedPoints(double px1, double py1, double px2, double py2,
        ...
        if (px1 == 0 && py1 == 0) {
            ...
            if (px2 != 0.0f && py2 != 0.0f) {
                ...
                float offset = calculateOffset(bw, bh, cyclic);
                rx2 = bx + offset;
                ry2 = by + offset;
            } else {
                //end point is either lower left corner --> vertical gradient
                //or end point is upper right corner --> horizontal gradient
                rx2 = (px2 == 0) ? rx1 : (cyclic ? (rx1 + bw / 2.0f) : (rx1 + bw));
                ry2 = (py2 == 0) ? ry1 : (cyclic ? (ry1 + bh / 2.0f) : (ry1 + bh));
            }
        } else {
    ...
    /**
     * Calculates the offset for diagonal gradients.
     *
     * @param bw      Target bounds width.
     * @param bh      Target bounds height.
     * @param cyclic Whether the gradient is cyclic.
     ...
     */
    private float calculateOffset(float bw, float bh, boolean cyclic) {
        return (cyclic) ? (bw + bh) / 4.0f : (bw + bh) / 2.0f;
    }
