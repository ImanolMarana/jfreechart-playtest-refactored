...
public class TextUtils {
    ...
    private static int nextLineBreak(String text, int start, float width, 
        ...
        while (end != BreakIterator.DONE) {
            ...
            if (x > width) {
                return handleWidthExceeded(text, start, width, firstWord,
                        end, iterator, measurer);
            } else {
                if (end > newline) {
                    return newline;
                }
            ...
            firstWord = false;
            current = end;
            end = iterator.following(current);
        }
        return BreakIterator.DONE;
    }

    private static int handleWidthExceeded(String text, int start, float width,
            boolean firstWord, int end, BreakIterator iterator, 
