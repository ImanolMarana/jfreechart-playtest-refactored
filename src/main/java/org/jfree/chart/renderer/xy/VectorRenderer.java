...
public class VectorRenderer extends AbstractXYItemRenderer
    ...
    public Range findRangeBounds(XYDataset dataset) {
        ...
        if (dataset instanceof VectorXYDataset) {
            return findRangeBoundsForVectorDataset((VectorXYDataset) dataset, seriesCount);
        } else {
            return findRangeBoundsForDefaultDataset(dataset, seriesCount);
        }
    }

    private Range findRangeBoundsForVectorDataset(VectorXYDataset vdataset, int seriesCount) {
        double minimum = Double.POSITIVE_INFINITY;
        double maximum = Double.NEGATIVE_INFINITY;
        for (int series = 0; series < seriesCount; series++) {
            ...
            for (int item = 0; item < itemCount; item++) {
                ...
                double uvalue = delta < 0.0 ? vdataset.getYValue(series, item) : vdataset.getYValue(series, item) + delta;
                double lvalue = delta < 0.0 ? uvalue + delta : vdataset.getYValue(series, item);
                minimum = Math.min(minimum, lvalue);
                maximum = Math.max(maximum, uvalue);
            }
        }
        return getRange(minimum, maximum);
    }

    private Range findRangeBoundsForDefaultDataset(XYDataset dataset, int seriesCount) {
        double minimum = Double.POSITIVE_INFINITY;
        double maximum = Double.NEGATIVE_INFINITY;
        for (int series = 0; series < seriesCount; series++) {
            int itemCount = dataset.getItemCount(series);
    ...
    private Range getRange(double minimum, double maximum){
        if (minimum > maximum) {
            return null;
        }
        else {
