...
public class AreaRenderer extends AbstractCategoryItemRenderer
    ...
    public void drawItem(Graphics2D g2, CategoryItemRendererState state,
        ...
        float x0 = (float) domainAxis.getCategoryStart(column, 
                dataset.getColumnCount(), dataArea, axisEdge);
        float x1 = (float) domainAxis.getCategoryMiddle(column, 
                dataset.getColumnCount(), dataArea, axisEdge);
        float x2 = (float) domainAxis.getCategoryEnd(column, 
                dataset.getColumnCount(), dataArea, axisEdge);
        ...
        double yy0 = calculateStartingY(this.endType, dataset, row, column, yy1);
        double yy2 = calculateEndingY(this.endType, dataset, row, column, yy1);
        ...
        GeneralPath area = createAreaPath(orientation, x0, x1, x2, y0, y1, 
                y2, yz);
    ...
    private double calculateStartingY(AreaRendererEndType endType,
            CategoryDataset dataset, int row, int column, double yy1) {
        double yy0 = 0.0;
        if (endType == AreaRendererEndType.LEVEL) {
            yy0 = yy1;
        }
        if (column > 0) {
    ...
    private double calculateEndingY(AreaRendererEndType endType,
            CategoryDataset dataset, int row, int column, double yy1) {
        double yy2 = 0.0;
        ...
        else if (endType == AreaRendererEndType.LEVEL) {
            yy2 = yy1;
        }
        return yy2;
    }

    private GeneralPath createAreaPath(PlotOrientation orientation, float x0,
            float x1, float x2, float y0, float y1, float y2, float yz) {
