...
public class BoxAndWhiskerRenderer extends AbstractCategoryItemRenderer
    ...
    public void drawHorizontalItem(Graphics2D g2, 
          ...
          double yy = calculateBoxYCoordinate(state, dataArea, categoryWidth,
                  seriesCount, categoryCount, row);

          drawBoxAndWhiskers(g2, state, plot, rangeAxis, bawDataset, row,
                  column, yy);

          drawMeanIndicator(g2, state, plot, rangeAxis, bawDataset, row,
                  column, yy);

          drawMedianIndicator(g2, state, plot, rangeAxis, bawDataset, row,
                  column, yy);

          collectEntityAndToolTipInfo(state, dataset, row, column);
      }

      private double calculateBoxYCoordinate(CategoryItemRendererState state,
                                           Rectangle2D dataArea, double categoryWidth, int seriesCount,
                                           int categoryCount, int row) {
          double yy;
          if (seriesCount > 1) {
              ...
              yy =  offset + (row * (state.getBarWidth() + seriesGap));
          } else {
      ...
      private void drawBoxAndWhiskers(Graphics2D g2,
                                     CategoryItemRendererState state, CategoryPlot plot,
                                     ValueAxis rangeAxis, BoxAndWhiskerCategoryDataset bawDataset,
                                     int row, int column, double yy) {
          ...
          if (xQ1 != null && xQ3 != null && xMax != null && xMin != null) {
              ...
              // draw the box...
              Shape box = new Rectangle2D.Double(Math.min(xxQ1, xxQ3), yy,
      ...
      private void drawMeanIndicator(Graphics2D g2,
                                    CategoryItemRendererState state, CategoryPlot plot,
                                    ValueAxis rangeAxis, BoxAndWhiskerCategoryDataset bawDataset,
                                    int row, int column, double yy) {
      ...
      private void drawMedianIndicator(Graphics2D g2,
                                      CategoryItemRendererState state, CategoryPlot plot,
                                      ValueAxis rangeAxis, BoxAndWhiskerCategoryDataset bawDataset,
                                      int row, int column, double yy) {
      ...
      private void collectEntityAndToolTipInfo(CategoryItemRendererState state,
                                              CategoryDataset dataset, int row, int column) {
          if (state.getInfo() != null) {
              EntityCollection entities = state.getEntityCollection();
              if (entities != null) {
                  addItemEntity(entities, dataset, row, column, null);
              }
          }
      }
