...
public abstract class AbstractCategoryItemRenderer extends AbstractRenderer
    ...
    public Object clone() throws CloneNotSupportedException {
        ...
        clone.itemLabelGeneratorMap = cloneMapValues(this.itemLabelGeneratorMap);

        clone.defaultItemLabelGenerator = cloneItemLabelGenerator(
                this.defaultItemLabelGenerator);

        clone.toolTipGeneratorMap = CloneUtils.cloneMapValues(
                this.toolTipGeneratorMap);

        clone.defaultToolTipGenerator = cloneCategoryToolTipGenerator(
                this.defaultToolTipGenerator);

        clone.itemURLGeneratorMap = CloneUtils.cloneMapValues(
                this.itemURLGeneratorMap);

        clone.defaultItemURLGenerator = cloneCategoryURLGenerator(
                this.defaultItemURLGenerator);
        
        clone.legendItemLabelGenerator = cloneLabelGenerator(
                this.legendItemLabelGenerator);
        clone.legendItemToolTipGenerator = cloneLabelGenerator(
                this.legendItemToolTipGenerator);
        clone.legendItemURLGenerator = cloneLabelGenerator(
                this.legendItemURLGenerator);
        return clone;
    }
    
    private <T> Map<Integer, T> cloneMapValues(Map<Integer, T> map)
            throws CloneNotSupportedException {
        if (map != null) {
            return CloneUtils.cloneMapValues(map);
        }
        return null;
    }
    
    private CategoryItemLabelGenerator cloneItemLabelGenerator(
            CategoryItemLabelGenerator generator) throws CloneNotSupportedException {
        if (generator != null) {
            if (generator instanceof PublicCloneable) {
                PublicCloneable pc = (PublicCloneable) generator;
                return (CategoryItemLabelGenerator) pc.clone();
            } else {
    ...
    private CategoryToolTipGenerator cloneCategoryToolTipGenerator(
            CategoryToolTipGenerator generator) throws CloneNotSupportedException {
        if (generator != null) {
            if (generator instanceof PublicCloneable) {
                PublicCloneable pc = (PublicCloneable) generator;
                return (CategoryToolTipGenerator) pc.clone();
            } else {
                throw new CloneNotSupportedException(
    ...
    private CategoryURLGenerator cloneCategoryURLGenerator(
            CategoryURLGenerator generator) throws CloneNotSupportedException {
        if (generator != null) {
            if (generator instanceof PublicCloneable) {
                PublicCloneable pc = (PublicCloneable) generator;
                return (CategoryURLGenerator) pc.clone();
            } else {
                throw new CloneNotSupportedException(
    ...
    private CategorySeriesLabelGenerator cloneLabelGenerator(
            CategorySeriesLabelGenerator generator) throws CloneNotSupportedException {
        if (generator instanceof PublicCloneable) {
            return (CategorySeriesLabelGenerator) CloneUtils.clone(
                    (Object) generator);
        }
        return null;
    }
