...
public class StackedBarRenderer extends BarRenderer
    ...
    public void drawItem(Graphics2D g2, CategoryItemRendererState state,
        ...
        PlotOrientation orientation = plot.getOrientation();
        double barW0 = calculateBarW0(domainAxis, dataArea, plot, state, column);
        ...
        RectangleEdge barBase = calculateBarBase(rangeAxis, orientation, value);
        ...
        Rectangle2D bar = createBar(orientation, barL0, barW0, barLength, state);

        if (pass == 0 && getShadowsVisible()) {
            boolean pegToBase = isPegToBase(positiveBase, negativeBase, value);
    ...
    private double calculateBarW0(CategoryAxis domainAxis, Rectangle2D dataArea, CategoryPlot plot, CategoryItemRendererState state, int column){
        return domainAxis.getCategoryMiddle(column, getColumnCount(),
                dataArea, plot.getDomainAxisEdge())
                - state.getBarWidth() / 2.0;
    }
    
    private RectangleEdge calculateBarBase(ValueAxis rangeAxis, PlotOrientation orientation, double value){
        RectangleEdge barBase;
        if (orientation == PlotOrientation.HORIZONTAL) {
            if ((value > 0.0 && rangeAxis.isInverted()) || (value < 0.0 && !rangeAxis.isInverted())) {
    ...
    private Rectangle2D createBar(PlotOrientation orientation, double barL0, double barW0, double barLength, CategoryItemRendererState state){
        if (orientation == PlotOrientation.HORIZONTAL) {
            return new Rectangle2D.Double(barL0, barW0, barLength,
                    state.getBarWidth());
        }
        else {
            return new Rectangle2D.Double(barW0, barL0, state.getBarWidth(),
                    barLength);
        }
    }

    private boolean isPegToBase(double positiveBase, double negativeBase, double value){
        return (value > 0.0 && (positiveBase == getBase()))
                || (value < 0.0 && (negativeBase == getBase()));
    }
