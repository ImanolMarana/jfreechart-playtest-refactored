...
public class LayeredBarRenderer extends BarRenderer implements Serializable {
    ...
    protected void drawVerticalItem(Graphics2D g2,
        ...
        double rectX = calculateBarX(domainAxis, state, column, dataArea, plot);
        ...
        // draw the bar...
        double shift = calculateSeriesShift(seriesCount);
        double widthFactor = calculateWidthFactor(row);
        ...
        drawBar(g2, state, dataset, row, column, bar);

        // draw the item labels if there are any...
        drawItemLabel(g2, state, dataset, row, column, plot, bar, value < base);

        // collect entity and tool tip information...
        collectEntity(state, dataset, row, column, bar);

    }
    
    private double calculateBarX(CategoryAxis domainAxis, CategoryItemRendererState state, 
            int column, Rectangle2D dataArea, CategoryPlot plot) {
        return domainAxis.getCategoryMiddle(column, getColumnCount(),
                dataArea, plot.getDomainAxisEdge()) - state.getBarWidth() / 2.0;
    }
    
    private double calculateSeriesShift(int seriesCount) {
        if (seriesCount > 1) {
            // needs to be improved !!!
            return  state.getBarWidth() * 0.20 / (seriesCount - 1);
        }
        return 0.0;
    }
    
    private double calculateWidthFactor(int row) {
        double seriesBarWidth = getSeriesBarWidth(row);
        if (!Double.isNaN(seriesBarWidth)) {
    ...
    private void drawBar(Graphics2D g2, CategoryItemRendererState state,
    ...
    private void drawItemLabel(Graphics2D g2, CategoryItemRendererState state, 
            CategoryDataset dataset, int row, int column, CategoryPlot plot, 
            Rectangle2D bar, boolean isNegative) {
        CategoryItemLabelGenerator generator = getItemLabelGenerator(row,
                column);
        if (generator != null && isItemLabelVisible(row, column)) {
            drawItemLabel(g2, dataset, row, column, plot, generator, bar,
    ...
    private void collectEntity(CategoryItemRendererState state, CategoryDataset dataset, 
