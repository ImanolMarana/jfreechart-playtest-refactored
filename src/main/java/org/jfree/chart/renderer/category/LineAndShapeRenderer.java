...
public class LineAndShapeRenderer extends AbstractCategoryItemRenderer
    ...
    public void drawItem(Graphics2D g2, CategoryItemRendererState state,
        ...
        // current data point...
        double x1 = calculateX(dataArea, column, visibleRow, state.getVisibleSeriesCount(), 
                domainAxis, dataset, plot);
        double value = v.doubleValue();
        double y1 = rangeAxis.valueToJava2D(value, dataArea,
                plot.getRangeAxisEdge());

        if (pass == 0 && getItemLineVisible(row, column) && column != 0) {
            drawLine(g2, dataArea, plot, domainAxis, rangeAxis, dataset, 
                    row, column, orientation, visibleRow, state.getVisibleSeriesCount(), x1, y1);
        }

        if (pass == 1) {
            drawShape(g2, orientation, row, column, x1, y1);
            drawItemLabel(g2, orientation, dataset, row, column, x1, y1, value);
            ...
            if (entities != null) {
                addItemEntity(entities, dataset, row, column, 
                        getItemShape(row, column));
            }
        }
    }

    private double calculateX(Rectangle2D dataArea, int column, int visibleRow, 
            int visibleRowCount, CategoryAxis domainAxis, CategoryDataset dataset,
            CategoryPlot plot) {
        if (this.useSeriesOffset) {
            return domainAxis.getCategorySeriesMiddle(column,
    ...
    private void drawLine(Graphics2D g2, Rectangle2D dataArea, CategoryPlot plot,
            CategoryAxis domainAxis, ValueAxis rangeAxis, CategoryDataset dataset, 
            int row, int column, PlotOrientation orientation, int visibleRow, 
            int visibleRowCount, double x1, double y1) {
        Number previousValue = dataset.getValue(row, column - 1);
        if (previousValue != null) {
            // previous data point...
            double previous = previousValue.doubleValue();
            double x0 = calculateX(dataArea, column - 1, visibleRow, 
                    visibleRowCount, domainAxis, dataset, plot);
    ...
    private void drawShape(Graphics2D g2, PlotOrientation orientation, int row,
        ...
        if (getItemShapeVisible(row, column)) {
            if (getItemShapeFilled(row, column)) {
                g2.setPaint(this.useFillPaint ? getItemFillPaint(row, column) :
                        getItemPaint(row, column));
                g2.fill(shape);
            }
            if (this.drawOutlines) {
                g2.setPaint(this.useOutlinePaint 
    ...
    private void drawItemLabel(Graphics2D g2, PlotOrientation orientation,
            CategoryDataset dataset, int row, int column, double x1, double y1, 
            double value) {
