    ...
    protected double[] getStackValues(CategoryDataset dataset,
        ...
        for (int i = 0; i < series; i++) {
            if (isSeriesVisible(i)) {
                result = addToStack(dataset, total, i, index, result);
            }
        }
        return result;
    }

    private double[] addToStack(CategoryDataset dataset, double total, int series,
            int index, double[] result) {
        double v = 0.0;
        Number n = dataset.getValue(series, index);
        if (n != null) {
            v = n.doubleValue();
            if (this.renderAsPercentages) {
                v = v / total;
