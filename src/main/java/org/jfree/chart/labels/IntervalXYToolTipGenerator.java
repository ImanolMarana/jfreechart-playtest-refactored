...
public class IntervalXYToolTipGenerator extends AbstractXYItemLabelGenerator
    ...
    protected Object[] createItemArray(XYDataset dataset, int series, 
        ...
        result[1] = formatValue(x, getXDateFormat(), getXFormat());
        result[2] = formatValue(xs, getXDateFormat(), getXFormat());
        result[3] = formatValue(xe, getXDateFormat(), getXFormat());

        result[4] = formatYValue(dataset, series, item, y, getYDateFormat(), getYFormat());
        result[5] = formatYValue(intervalDataset, series, item, ys, getYDateFormat(), getYFormat(), true);
        result[6] = formatYValue(intervalDataset, series, item, ye, getYDateFormat(), getYFormat(), false);

        return result;
    }

    private String formatValue(double value, DateFormat dateFormat, NumberFormat numberFormat) {
        if (dateFormat != null) {
    ...
    private String formatYValue(XYDataset dataset, int series, int item, double yValue, 
                                 DateFormat dateFormat, NumberFormat numberFormat) {
        return formatYValue(dataset, series, item, yValue, dateFormat, numberFormat, false);
    }

    private String formatYValue(XYDataset dataset, int series, int item, double yValue, 
                                 DateFormat dateFormat, NumberFormat numberFormat, boolean isStart) {
        if (Double.isNaN(yValue) && 
            dataset != null && 
            (isStart ? dataset.getStartY(series, item) : dataset.getEndY(series, item)) == null) {
            return getNullYString();
        } else {
            return formatValue(yValue, dateFormat, numberFormat);
        }
    }
//Refactoring end
    }
